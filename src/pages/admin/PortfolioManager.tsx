...
const CATEGORIES = [
  'Wedding', 'Haldi', 'Pre-Wedding', 'Bike Shoots', 'Reels',
  'Cinematic Edits', 'Social Media Content',
];
...
export function PortfolioManager() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      console.error('PortfolioManager delete error:', e);
  ...
  const togglePublish = async (row: PortfolioItem) => {
    try { await update(row.id, { is_published: !row.is_published }); await log('update', 'portfolio', row.id, { is_published: !row.is_published }); }
    catch (e) { console.error('PortfolioManager togglePublish error:', e); setFormError(e instanceof Error ? e.message : 'Publish toggle failed'); }
  };
  ...
  return (
    <div>
      <PageHeader
        title="Portfolio"
        subtitle="Manage all portfolio items — create, edit, publish, delete."
        action={
      ...
      <div className="p-6">
        <DataTable<PortfolioItem>
          ...
          columns={[
            {
              key: 'thumbnail_url', label: 'Image', width: '72px',
              render: (r) => r.thumbnail_url ? (
                <img src={r.thumbnail_url} alt={r.title} className="h-12 w-16 rounded-lg object-cover" loading="lazy" />
              ) : <div className="h-12 w-16 rounded-lg bg-ink-800" />,
            },
      ...
      <Modal open={formOpen} onClose={closeForm} title={editing ? 'Edit Portfolio Item' : 'Add Portfolio Item'}>
        <div className="space-y-4">
          ...
          {form.thumbnail_url && (
            <img src={form.thumbnail_url} alt="Preview" className="h-24 w-full rounded-xl object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={save} disabled={saving} className="btn-primary flex-1 disabled:opacity-60">
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2 text-sm text-stone-300">
      <button
        ...
        className={`relative h-5 w-9 rounded-full transition-colors ${checked ? 'bg-gold-500' : 'bg-ink-700'}`}
      >
