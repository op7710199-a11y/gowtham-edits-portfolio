...
export function FaqsManager() {
  ...
  const confirmDelete = async () => {
    ...
    catch (e) { console.error('FaqsManager delete error:', e); setFormError(e instanceof Error ? e.message : 'Delete failed'); }
    finally { setDeleting(false); }
  };

  const togglePublish = async (row: FaqItem) => {
    try { await update(row.id, { is_published: !row.is_published }); }
    catch (e) { console.error('FaqsManager togglePublish error:', e); setFormError(e instanceof Error ? e.message : 'Publish toggle failed'); }
  };
