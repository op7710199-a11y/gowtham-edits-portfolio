  ...
  return (
    <section id="services-preview" className="section-padding relative">
      <div className="container-mx">
        <SectionHeading eyebrow="What I Edit" title={<>Services built around <span className="text-gradient-gold">your story</span></>}
        ...
        <RevealScope className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {preview.map((s, i) => (
            <Reveal key={s.id ?? i} delay={i * 90} as="article">
              <div className="card-glass group h-full p-6 hover:border-gold-500/30 hover:bg-white/[0.06]">
                <div className="grid h-12 w-12 place-items-center rounded-xl bg-gold-500/10 text-gold-300 transition-colors group-hover:bg-gold-gradient group-hover:text-ink-950">
                  <Icon name={(s.icon ?? 'Film') as IconName} className="h-6 w-6" />
                </div>
                ...
                <a href="#services" className="mt-5 inline-flex items-center gap-1 text-sm font-medium text-gold-300 transition-colors hover:text-gold-100">
        ...
        <Reveal className="mt-10 text-center">
          <a href="#services" className="btn-ghost">View all {items.length} services <ArrowUpRight className="h-4 w-4" /></a>
        </Reveal>
      </div>
    </section>
  );
